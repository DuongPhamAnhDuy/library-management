...
INSERT INTO THUTHU
VALUES ('TT001','HOANG',N'LÊ SĨ HOÀNG',N'NAM','10/04/1987','0915481325','LESIHOANG@GMAIL.COM',N'5 NGUYỄN SÁNG- TÂY THẠNH- TÂN PHÚ'),
('TT002','HUNG',N'LÊ TIẾN HƯNG',N'NAM','10/03/1975','0354861746','LETIENHUNG@GMAIL.COM',N'152 NGUYỄN HUỆ- QUẬN 1'),
('TT003','ANHDUY',N'DƯƠNG PHẠM ANH DUY',N'NAM','17/10/1995','034865267','ANHDUY@GMAIL.COM',N'364 NGUYỄN CỪ- QUẬN 3'),
('TT004','THANG',N'ĐẶNG QUỐC THẮNG',N'NAM','19/7/1998','037213773','THANG@GMAIL.COM',N'334 NGUYỄN ẢNH THỦ')
...
-- Tống số đọc giả --
DROP PROCEDURE tongsodocgia

CREATE PROCEDURE tongsodocgia
    @so INT OUTPUT
AS
BEGIN
...
EXEC tongsodocgia @so = @result OUTPUT

SELECT @result AS TotalDocGia
...
-- Tổng số sách --
CREATE PROCEDURE tongsosach
AS
BEGIN
    SELECT COUNT(*) AS SoLuongSach
    FROM SACH
END
-- Tổng Phiếu mượn --
CREATE PROCEDURE tongsophieumuon
AS
BEGIN
...
CREATE PROCEDURE tongsosachdamuon
AS
BEGIN
...
CREATE PROCEDURE tongsosachconlai
AS
BEGIN
    SELECT COUNT(*) AS SoLuongSachConLai
    FROM SACH
    WHERE TINHTRANG = N'KHÔNG MƯỢN'
END
-- Tổng số phiếu quá hạn --
CREATE PROCEDURE tongsophieuquahan
AS
BEGIN
    SELECT COUNT(*) AS SoLuongPhieuQuaHan
    FROM MUONTRA
